...
test('distribution extension validates inside the canonical Asset Registry', () => {
  ...
  assert.match(result.stdout, /580 verified counted placements/);
});
...
test('distribution baseline separates works, representations, placements, releases and ownership', () => {
  ...
  assert.equal(data.counts.verified_counted_placements, 580);
  assert.equal(data.counts.gap_to_minimum, 920);
...
test('distribution schema gates verified placement evidence', () => {
  ...
  for (const field of ['work_id', 'placement_id', 'release_id', 'ownership_group', 'public_url', 'status', 'verification']) assert.ok(required.includes(field));
});
