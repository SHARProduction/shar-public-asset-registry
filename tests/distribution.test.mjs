...
test('distribution extension validates inside the canonical Asset Registry', () => {
  ...
  assert.match(result.stdout, /638 verified counted placements/);
});
...
test('distribution baseline separates works, representations, placements, releases and ownership', () => {
  ...
  assert.equal(data.counts.verified_counted_placements, 638);
  assert.equal(data.counts.gap_to_minimum, 862);
  assert.equal(data.works.length, 26);
  assert.equal(data.representations.length, 52);
  assert.equal(data.placements.length, 59);
