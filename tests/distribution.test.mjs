...
test('distribution extension validates inside the canonical Asset Registry', () => {
  ...
  assert.match(result.stdout, /660 verified counted placements/);
});
...
test('distribution baseline separates works, representations, placements, releases and ownership', () => {
  ...
  assert.equal(data.counts.verified_counted_placements, 660);
  assert.equal(data.counts.gap_to_minimum, 840);
  assert.equal(data.works.length, 36);
  assert.equal(data.representations.length, 72);
  assert.equal(data.placements.length, 81);
