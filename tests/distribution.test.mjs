...
test('distribution extension validates inside the canonical Asset Registry', () => {
  ...
  assert.match(result.stdout, /942 verified counted placements/);
});
...
test('distribution baseline separates works, representations, placements, releases and ownership', () => {
  ...
  assert.equal(data.counts.verified_counted_placements, 942);
  assert.equal(data.counts.gap_to_minimum, 558);
  assert.equal(data.works.length, 168);
  assert.equal(data.representations.length, 336);
  assert.equal(data.placements.length, 363);
