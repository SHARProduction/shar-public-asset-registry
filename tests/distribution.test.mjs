...
test('distribution extension validates inside the canonical Asset Registry', () => {
  ...
  assert.match(result.stdout, /994 verified counted placements/);
});
...
test('distribution baseline separates works, representations, placements, releases and ownership', () => {
  ...
  assert.equal(data.counts.verified_counted_placements, 994);
  assert.equal(data.counts.gap_to_minimum, 506);
  assert.equal(data.works.length, 193);
  assert.equal(data.representations.length, 386);
  assert.equal(data.placements.length, 415);
