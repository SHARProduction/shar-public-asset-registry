...
test('distribution extension validates inside the canonical Asset Registry', () => {
  ...
  assert.match(result.stdout, /1046 verified counted placements/);
});
...
test('distribution baseline separates works, representations, placements, releases and ownership', () => {
  ...
  assert.equal(data.counts.verified_counted_placements, 1046);
  assert.equal(data.counts.gap_to_minimum, 454);
  assert.equal(data.works.length, 218);
  assert.equal(data.representations.length, 436);
  assert.equal(data.placements.length, 467);
