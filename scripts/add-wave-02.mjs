...
const waveRoot=path.resolve(root,'../wave-02/repos');
...
for(const name of dirs){
  ...
  const assetId=`shar-public.asset.github.wave02.${name}`;
...
const siteId='shar-public.asset.web.wave02';
if(!existing.has(siteId))registry.assets.push({
  asset_id:siteId,asset_type:'website',title:'SHAR Production Public Engine Wave 02',title_ru:'Вторая волна SHAR Production Public Engine',owner:'SHAR Production',brand:{name:'SHAR Production',website:'https://sharprod.com/'},provider:'Cloudflare Pages',namespace:'bullolaya/shar-production-wave-02',public_url:waveUrl,canonical_url:waveUrl,version:{kind:'content_sha256',value:crypto.createHash('sha256').update(body).digest('hex')},visibility:'public',license:'MIT',status:'PUBLISHED_VERIFIED',evidence_class:'LIVE_HTTP_CHECK',verified_at:checkedAt,verification_method:'Live crawl of all 62 public pages plus desktop/mobile and browser-function checks',verification:{outcome:'PASS',http_status:200,content_checked:true,function_checked:true,owner_checked:true,version_checked:true,details:['All 62 EN/RU pages returned HTTP 200 with unique exact canonicals, indexable robots metadata and valid JSON-LD.','Root and sitemap expose 20 tools in 10 families; robots.txt and sitemap.xml returned HTTP 200.','Playwright at an exact 390px viewport measured innerWidth, document scrollWidth and body scrollWidth as 390; a live lens calculator returned valid=true with numeric results.']},languages:['ru','en'],relationships:[{relation:'links_to',target:'shar-public.asset.web.sharprod-home'},{relation:'links_to',target:'shar-public.asset.github.open-tools'}],content_summary:'Primary public Wave 02 deployment with 20 bilingual browser tools and zero trackers.',notes:[],next_review:nextReview
});
const orchestratorId='shar-public.asset.github.wave02-orchestrator';
if(!existing.has(orchestratorId))registry.assets.push({
  asset_id:orchestratorId,asset_type:'github_repository',title:'SHAR Production Public Engine Wave 02 Orchestrator',title_ru:'Оркестратор второй волны SHAR Production Public Engine',owner:'SHAR Production',brand:{name:'SHAR Production',website:'https://sharprod.com/'},provider:'GitHub',namespace:'SHARProduction/shar-public-engine-wave-02',public_url:'https://github.com/SHARProduction/shar-public-engine-wave-02',canonical_url:'https://github.com/SHARProduction/shar-public-engine-wave-02',version:{kind:'release_tag',value:'v1.0.0'},visibility:'public',license:'MIT',status:'PUBLISHED_VERIFIED',evidence_class:'PUBLIC_CLONE_CHECK',verified_at:checkedAt,verification_method:'Fresh recursive public clone plus full manifest and repository tests',verification:{outcome:'PASS',http_status:200,content_checked:true,function_checked:true,owner_checked:true,version_checked:true,details:['Public v1.0.0 release is non-draft and non-prerelease.','Fresh recursive clone resolved all 10 pinned submodules at their v1.0.3 commits.','Fresh-clone test suite passed 5/5 and covered 20 distinct evaluators and 60 localized pages.']},languages:['ru','en'],relationships:[{relation:'links_to',target:'shar-public.asset.web.wave02'},{relation:'links_to',target:'shar-public.asset.web.sharprod-home'}],content_summary:'Reproducible source and verification orchestrator for Wave 02.',notes:[],next_review:nextReview
});
